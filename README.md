...
| `SHAKE_KNOWN_ANTHRO_TOPIC` | MQTT topic to subscribe for known anthropogenic events (`{"type":"garage_door"\|"none","expected_duration":<sec>}`) used to annotate history `source` | `Shake/known_anthropogenic_event` |
...
- **`ShakeMqttBridge`** ([shake_mqtt/bridge.py](shake_mqtt/bridge.py)) — accepts an optional `processor=` argument for dependency injection; when **`SHAKE_CATALOG_ENABLE`** is set, trigger start/peak-update messages immediately refresh trigger-derived **`{MQTT_TOPIC}/match/ratio`**, **`/sta_rms`**, and **`/lta_rms`** (with `catalog_present=0` until a catalog row is found). It still schedules one USGS lookup per entry in **`SHAKE_CATALOG_QUERY_OFFSETS_SEC`** (default **1, 2, and 3 minutes** after trigger start) and publishes **leaf topics** under **`{MQTT_TOPIC}/match/`** after each attempt (see `shake_mqtt/topic_publish.py`). Peak update trigger messages do not schedule extra catalog lookups. With travel time enabled, the chosen event minimizes the gap between the trigger time and **origin time + IRIS first-arrival travel time**.
- **Known anthropogenic source annotation** — the bridge subscribes to **`SHAKE_KNOWN_ANTHRO_TOPIC`** (default `Shake/known_anthropogenic_event`) and accepts payloads like `{"type":"garage_door","expected_duration":10}` or `{"type":"none","expected_duration":0}`. A non-`none` update creates an annotation window at `update_time +/- expected_duration` seconds. Trigger rows whose trigger `time` falls inside that window are tagged with a human-readable `source` (`garage_door` -> `Garage Door`). Triggers tagged this way skip USGS/IRIS external matching lookups.
- **Match history** ([shake_mqtt/match_history.py](shake_mqtt/match_history.py)) — when catalog and **`SHAKE_MATCH_HISTORY_ENABLE`** are on, each trigger start/peak update and each completed lookup updates retained JSON on **`{MQTT_TOPIC}/match/history_json`**: `{"matches":[...],"updated_unix":...}`. Each row includes `shakenet_window_url` built from trigger time +/- the configured window and a `source` field (`Garage Door` when a known anthropogenic window matches that trigger time, `null` otherwise). Trigger fields (**`ratio`**, **`sta_rms`**, **`lta_rms`**) always come from the trigger; catalog fields mirror MQTT leaf topics. Rows with a catalog event also include **`distance_mi`** (great-circle epicenter distance from **`SHAKE_CATALOG_LATITUDE` / `LONGITUDE`**) and **`delta_magnitude`** (event magnitude minus the minimum magnitude implied by that distance on the same sensitivity curve used for optional filtering, including **`SHAKE_CATALOG_SENSITIVITY_MAGNITUDE_OFFSET`**; **`null`** when magnitude is missing). **`matches`** is sorted by **`sta_rms`** (descending). Example MQTT sensor + Markdown dashboard table: [homeassistant/shake_event_template.yaml](homeassistant/shake_event_template.yaml) (comments) and the snippet below.
...
For known anthropogenic source tagging from garage door state changes, see:
[`homeassistant/shake_known_anthro_garage_door_automation.yaml`](homeassistant/shake_known_anthro_garage_door_automation.yaml)
