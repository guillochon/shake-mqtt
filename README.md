...
- **Match history** ([shake_mqtt/match_history.py](shake_mqtt/match_history.py)) — when catalog and **`SHAKE_MATCH_HISTORY_ENABLE`** are on, each completed lookup updates **`{MQTT_TOPIC}/match/history_json`**: retained JSON `{"matches":[...],"updated_unix":...}`. Each row also includes `shakenet_window_url` built from trigger time +/- the configured window. Catalog fields mirror MQTT leaf topics; **`sta_rms`** is always the trigger value (so sorts and plots work when USGS returns no row, unlike **`Shake/match/sta_rms`**, which is empty then). **`matches`** is sorted by **`sta_rms`** (descending). Example MQTT sensor + Markdown dashboard table: [homeassistant/shake_event_template.yaml](homeassistant/shake_event_template.yaml) (comments) and the snippet below.
